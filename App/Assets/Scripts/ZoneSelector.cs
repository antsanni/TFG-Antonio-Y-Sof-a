...
public class ZoneSelector : MonoBehaviour {
...

    {
        ...
        if (!wsClient) wsClient = FindObjectOfType<DroneWSClient>();
...

        {
...

            {
...

                {
                    ...
                    wsClient.SendSetHome(hit.point.x, hit.point.y, terrainMSL);
...

    {
        ...
        //if (wsClient) wsClient.SendMission(wps);
     ...
     public void ExportMission()
    {
        ...
        if (wpsTotales.Count > maxWaypoints)
        {
            ...
            {
                int idx = Mathf.RoundToInt(i * ratio);
                if (idx >= wpsTotales.Count) idx = wpsTotales.Count - 1;
